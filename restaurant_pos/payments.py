...
class Order:
    ...
    def subtotal(self) -> Decimal:
        return sum((item.line_total() for item in self.items), Decimal("0")).quantize(CENT, rounding=ROUND_HALF_UP)
    ...
    def calculate_totals(self) -> dict:
        ...
        discount_amount = (subtotal * (self.discount_percentage / Decimal("100"))).quantize(CENT, rounding=ROUND_HALF_UP)
        discounted_subtotal = (subtotal - discount_amount).quantize(CENT, rounding=ROUND_HALF_UP)
    ...
    def receipt_lines(self) -> List[str]:
        ...
        if not self.items:
            ...
            lines.extend(["", f"Subtotal: ${totals['subtotal']:.2f}", f"Tax: ${totals['tax']:.2f}", f"Total: ${totals['total']:.2f}"])
            return lines
...
class PaymentProcessor:
    ...
    def process_payment(self, order: Order, tendered_amount: float | Decimal, *, trigger_failure: bool = False) -> dict:
        try:
